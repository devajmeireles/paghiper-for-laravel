...
Route::post('/payment/notification', function (Request $request) {
    ...
    $status = PagHiper::billet()->notification($notification, $transaction);
})->name('paghiper.notification');
```

No exemplo acima, `$status` será um array com os dados da notificação.
...
Route::post('/payment/notification', function (Request $request) {
    $status = PagHiper::billet()->notification($request);
})->name('paghiper.notification');
```
...
Os demais métodos seguem a convenção de nome da PagHiper:
...
No exemplo acima, `$status` será uma instância da classe `PagHiperBilletNotification` contendo o método `modelable()`.
Ao utilizar o método `$status->modelable()` `PagHiper for Laravel` irá recuperar o usuário automaticamente:

```php
$user = $status->modelable(); // 👈
```
